...
public class FdcMybatisTest {
    ...
    private static final Logger logger = LoggerFactory.getLogger(FdcMybatisTest.class);
    ...
    public static void main( String[] args )
    {
        ...
        logger.info("====LGD Phoenix myBatis DevCode====");
        fdcService = new FdcService();

        logger.info("== select all fdc test ==");
        List<FdcTest> fdcTestList = fdcService.findAllFdcTest();
        for (FdcTest fdcTest : fdcTestList){
            logger.info("\tRow: " + fdcTest.getMyKey() + " = " + fdcTest.getMyColumn());
        }

        logger.info("== select fdc test by id 2==");
        FdcTest fdcTest2 = fdcService.findFdcTestById(2);
        logger.info("\tRow: " + fdcTest2.getMyKey() + " = " + fdcTest2.getMyColumn());

        FdcTest fdcTest3 = new FdcTest(5,"5th fdc test record");
        logger.info("== insert fdc test by id 5==");
        fdcService.createFdcMapper(fdcTest3);

        logger.info("== select all fdc test again ==");
